...
interface BottomNavigationProps {
  ...
  onOpenAuth: () => void;
  onLogout: () => void;
  currentTab: 'menu' | 'orders' | 'admin' | 'profile';
  setCurrentTab: (t: 'menu' | 'orders' | 'admin' | 'profile') => void;
}
...
export default function BottomNavigation({
  ...
  const handleTabChange = (tab: 'menu' | 'orders' | 'admin' | 'profile') => {
    if (tab === 'orders' && !user) {
      onOpenAuth();
    } else {
      setCurrentTab(tab);
            ...
            >
              ...
              <div className="space-y-3.5 mb-6">
                ...
                <button
                  ...
                  className="w-full bg-white rounded-2xl p-4 border border-[#E5E2D9] flex justify-between items-center text-right hover:bg-gray-50 transition-all duration-200 cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-[#af301d]/5 text-[#af301d] flex items-center justify-center border border-[#af301d]/15">
                      <Shield className="w-4 h-4" />
                    </div>
                    <div>
                      <span className="text-xs font-black text-brand-primary block">
                        {isAr ? 'الأمان وإعدادات الحساب' : 'Security & Account Profile'}
                      </span>
                      <span className="text-[10px] font-bold text-gray-400">
                        {isAr ? 'تحديث اسم المستخدم وكلمة المرور' : 'Change password & credentials'}
                      </span>
                    </div>
                  </div>
