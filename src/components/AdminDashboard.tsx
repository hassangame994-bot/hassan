...
import { Order, OrderStatus, DashboardStats, MenuItem, Category, User } from '../types.js';
import { playStatusUpdateChime } from '../utils/audio.js';
import AdminAccountsPanel from './AdminAccountsPanel.js';
  ...
  const executeDelete = async () => {
    ...
    try {
      if (type === 'order') {
        ...
        const res = await fetch(`/api/menu/${targetId}`, { 
          method: 'DELETE',
          headers: {
            'x-user-id': user?.id || '',
            'Authorization': user?.token ? `Bearer ${user.token}` : ''
    ...
    <div className="w-full text-[#2D241E]" dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="flex border-b border-[#E5E2D9] mb-8 gap-1 sm:gap-6 overflow-x-auto no-scrollbar">
        ...
        >
          ...
          <span>{isAr ? 'الأطباق والأصناف' : 'Menu Items'}</span>
        </button>
        ...
        >
          ...
          <span>{isAr ? 'أقسام المنيو' : 'Food Categories'}</span>
        </button>

        {(user?.isSuperAdmin || user?.id === 'admin-1') && (
          <button
            onClick={() => setActiveTab('accounts' as any)}
            className={`pb-4 px-2 text-xs sm:text-sm font-extrabold transition-all border-b-2 flex items-center gap-2 cursor-pointer whitespace-nowrap ${
              activeTab as string === 'accounts'
                ? 'border-[#3D4021] text-[#3D4021]'
                : 'border-transparent text-gray-400 hover:text-[#3D4021]'
            }`}
          >
            <Shield className="w-4 h-4 text-[#f3a216]" />
            <span>{isAr ? 'الحسابات والموظفين' : 'Accounts & Staff'}</span>
          </button>
        )}
      </div>
      ...
      {activeTab as string === 'accounts' && (user?.isSuperAdmin || user?.id === 'admin-1') && (
        <AdminAccountsPanel
          user={user}
          lang={lang}
