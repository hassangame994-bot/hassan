...
export default function CartDrawer({
  ...
  const [locType, setLocType] = useState<'none' | 'gps' | 'ip'>(user?.latitude ? 'gps' : 'none');
  ...
  useEffect(() => {
    if (user) {
      ...
      setLocType(user.latitude ? 'gps' : 'none');
  ...
  const handleCaptureGPS = () => {
    if (!navigator.geolocation) {
      // Fallback directly to IP geolocation if browser doesn't support navigator.geolocation
      setLocating(true);
      setError('');
      tryIPGeolocation();
      return;
    ...
    // High Availability / Resilient Cascade Strategy:
    // Stage 1: Try GPS with high accuracy and a short 4.5 second timeout
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        console.warn('Stage 1 GPS High Accuracy failed/timed out. Cascading to Stage 2 (Low Accuracy)...', err);
        
        // Stage 2: Fallback to Low Accuracy (Wi-Fi / Cell tower triangulation) with a 6 second timeout
        navigator.geolocation.getCurrentPosition(
          (position) => {
            const lat = position.coords.latitude;
            const lng = position.coords.longitude;
          ...
          (err2) => {
            console.warn('Stage 2 GPS Low Accuracy failed. Cascading to Stage 3 (Secure IP Geolocation)...', err2);
            
            // Stage 3: Fallback to Secure IP-based Geolocation lookup
            tryIPGeolocation();
          },
  ...
  // Helper for Stage 3 Secure IP Geolocation
  const tryIPGeolocation = async () => {
    // Try multiple free secure APIs for maximum redundancy
    const apis = [
      { url: 'https://ipapi.co/json/', latKey: 'latitude', lngKey: 'longitude' },
      { url: 'https://ip-api.com/json/', latKey: 'lat', lngKey: 'lon' }
    ];
    ...
    for (const api of apis) {
      try {
        ...
        if (res.ok) {
          ...
          const lat = data[api.latKey];
          const lng = data[api.lngKey];
          ...
          if (lat && lng && typeof lat === 'number' && typeof lng === 'number') {
            ...
            // Autocomplete address details if the user hasn't written any
            if (!address.trim()) {
              const city = data.city || '';
              const region = data.region || data.region_name || '';
              if (city || region) {
                const autoAddr = isAr 
                  ? `تحديد تقريبي: ${city} ${region}`.trim()
                  : `Approx: ${city}, ${region}`.trim();
                setAddress(autoAddr);
              }
            }
            ...
            if (window.leafletMapInstance && window.leafletMarkerInstance) {
              // @ts-ignore
              window.leafletMapInstance.setView([lat, lng], 14); // Zoom slightly out since IP location is less accurate
              // @ts-ignore
              window.leafletMarkerInstance.setLatLng([lat, lng]);
            }
            return;
        ...
        console.warn(`IP Geolocation fallback for ${api.url} failed:`, e);
      }
    }

    // If everything completely failed
    setLocating(false);
    setError(isAr 
      ? 'فشل تحديد الموقع التلقائي. تأكد من تشغيل الـ GPS أو إعطاء الصلاحية، ويمكنك سحب الدبوس يدوياً على الخريطة.' 
      : 'Could not fetch coordinates. Please enable GPS/allow permissions, or manually drag the map pin.');
  };
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <button
                      ...
                      className={`w-full py-3.5 rounded-2xl border flex items-center justify-center gap-2 transition-all text-xs font-black cursor-pointer shadow-sm ${
                        locating 
                          ? 'bg-[#F1EFE7]/30 border-[#E5E2D9] text-gray-400'
                          : locSuccess 
                            ? locType === 'gps'
                              ? 'bg-emerald-50 border-emerald-200 text-emerald-800 hover:bg-emerald-100/85'
                              : locType === 'ip'
                                ? 'bg-amber-50 border-amber-200 text-amber-800 hover:bg-amber-100/85'
                                : 'bg-teal-50 border-teal-200 text-teal-800 hover:bg-teal-100/85'
                      ...
                      {locating ? (
                        ...
                        locType === 'gps' ? (
                          <>
                            ...
                            <span>
                              {isAr 
                                ? 'تم تحديد موقعك بدقة عبر الـ GPS 🛰️' 
                                : 'Exact GPS Location Captured 🛰️'}
                            </span>
                          </>
                        ) : locType === 'ip' ? (
                          <>
                            ...
                            <span>
                              {isAr 
                                ? 'تم التقاط موقعك التقريبي عبر الشبكة 🌐' 
                                : 'Approx Network Location Captured 🌐'}
                            </span>
                          </>
                        ) : (
                          <>
                            ...
                            <span>
                              {isAr 
                                ? 'تم حفظ موقعك بدقة على الخريطة 📍' 
                                : 'Location Pin Set on Map 📍'}
                            </span>
                          </>
                        )
                      ) : (
                    ...
                    <div className="px-3.5 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-[10px] text-gray-500 font-semibold font-mono flex flex-wrap gap-2 justify-between">
                      ...
                      <span className={`${locSuccess ? 'text-emerald-600' : 'text-gray-400'} font-bold`}>
                        {locSuccess 
                          ? locType === 'ip' 
                            ? (isAr ? '[موقع الشبكة]' : '[Network IP]') 
                            : (isAr ? '[مؤمن GPS]' : '[Secure GPS]')
                          : (isAr ? '[غير محدد]' : '[Not Set]')}
                      </span>
                    </div>

                    {/* Highly Professional Guidance for IP Fallback */}
                    {locSuccess && locType === 'ip' && (
                      <div className="p-3 bg-amber-50/60 border border-amber-100 rounded-2xl text-[10px] text-amber-700 font-bold leading-relaxed text-center animate-in fade-in slide-in-from-top-2 duration-300">
                        {isAr 
                          ? '📍 نظراً لتعذر تشغيل الـ GPS حالياً، حددنا موقعك التقريبي تلقائياً عبر الإنترنت. للحصول على خط سير دقيق لدليفري أبو قورة، يُرجى سحب الدبوس الأحمر فوق منزلك بالتحديد.' 
                          : '📍 Geolocation is currently restricted. We have approximate-pinned your location via Network IP. For exact delivery directions, please drag the red map pin directly over your house.'}
                      </div>
                    )}
                  </div>
